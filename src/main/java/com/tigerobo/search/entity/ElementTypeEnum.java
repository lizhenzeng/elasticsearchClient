...
public enum ElementTypeEnum {
    ...
    EQUAL(Constant.equal) {
        ...
        public QueryBuilder buildQueryBuilder(Element element, MetaDataStatement mds, QueryBuilder f1, QueryBuilder f2) {
            ...
            BoolQueryBuilder middleBool = QueryBuilders.boolQuery();
            if (element.getClass().isAssignableFrom(SymbolMeta.class) && !element.getParamterName().equalsIgnoreCase(Constant.one)) {
                ...
                if (Validation.notEmptyAndBlankStr(((SymbolMeta) element).getGroupName())) {
                    if (element.getCompareValue().contains(",")) {

                        String[] vars = element.getCompareValue().split(",");
                        Integer var = vars.length;
                        middleBool.must(QueryBuilders.termsQuery(String.format("%s.%s",
                                ((SymbolMeta) element).getGroupName(), ((SymbolMeta) element).getDetailName())
                                , element.getCompareValue().split(",")));
                        middleBool.filter(QueryBuilders.scriptQuery(new Script("doc['" + String.format("%s.%s",
                                ((SymbolMeta) element).getGroupName(), ((SymbolMeta) element).getDetailName()) + "'].values.length == " + var)));
                        NestedQueryBuilder nqb = QueryBuilders.nestedQuery(((SymbolMeta) element).getGroupName(), middleBool, ScoreMode.None);
                    ...
                    if (element.getCompareValue().contains(",")) {
                        String[] vars = element.getCompareValue().split(",");
                        Integer var = vars.length;
                        middleBool.filter(QueryBuilders.scriptQuery(new Script("doc['" + String.format("%s",
                                element.getParamterName() + "'].values.length == " + var))));
                        middleBool.must(QueryBuilders.termsQuery(element.getParamterName(), vars));
                    } else {

                        middleBool.must(QueryBuilders.termsQuery(element.getParamterName(), element.getCompareValue()));
                    }


                    boolQueryBuilder.must(middleBool);
                }
            }
            return boolQueryBuilder;
        }
            ...
            if (element.getClass().isAssignableFrom(SymbolMeta.class)) {
                ...
                if (Validation.notEmptyAndBlankStr(((SymbolMeta) element).getGroupName())) {
                    ...
                    if (element.getCompareValue().contains(",")) {
                        ...
                        NestedQueryBuilder nqb = QueryBuilders
                                .nestedQuery(((SymbolMeta) element).getGroupName(), QueryBuilders.boolQuery().must(
                                        QueryBuilders.termsQuery(String.format("%s.%s",
                                        ((SymbolMeta) element).getGroupName(), ((SymbolMeta) element).getDetailName())
                                        , element.getCompareValue())), ScoreMode.None);
                        boolQueryBuilder.must(nqb);
                    }
